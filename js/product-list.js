class ProductList {
      ...
      .forEach((product) => {
        productListDomString += `<div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                  <div class="card product">
                    <img class="card-img-top" src="image/products/${product.image}" 
                        alt="${product.title}">
                    <div class="card-body d-flex flex-column">
                      ...
                      <div class="d-flex justify-content-around">
                        <button class="btn btn-info" data-toggle="modal"
                          data-target="#productInfoModal" data-id="${product.id}">Инфо
                        </button>
                        <button class="btn btn-primary buy" data-id="${product.id}">
                          ${product.price} грн - Купить
                        </button>
                      </div>
                    </div>
                  </div>
  ...
  async handleProductInfoClick(event) {
    ...
    productImg.setAttribute("src", "image/products/" + product.image);
    ...
    btnBuy.innerText = `${product.price} - Купить`; /*  Купить поменять на Buy*/
