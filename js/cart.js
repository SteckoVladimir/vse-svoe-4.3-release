class Cart {
  ...
  async renderCart() {
    ...
    let cartDomSting = `<div class="container">
                <div class="row">
                    <div class="col-5"><strong>Продукция</strong></div>
    ...
    cartDomSting += `
                <div class="row">
                    <div class="col-5"><strong>Общая сумма</strong></div>
  ...
  order(ev) {
    if (this.cartLength() === 0) {
      window.showAlert("Пожалуйста, выберите товары для заказа", false);
      return;
    }    
      ...
      fetch('order', {
        ...
        .then(response => {
          if (response.status === 200) {
            ...
            throw new Error("Невозможно отправить форму");
          }
        })
        .then(responseText => {
          form.reset();
          ...
          this.updateBadge();
          this.renderCart();
          window.showAlert("Спасибо за заказ! " + responseText);
          this.cartContainer.querySelector('.btn-close').click();
        })
        .catch(error => showAlert('There is an error: ' + error, false));
    } else {
      window.showAlert("Пожалуйста, заполните форму правильно", false);
    }
  }
}
